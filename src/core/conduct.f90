...
CONTAINS
  ...
  SUBROUTINE Conduct_Heat
    REAL(num),DIMENSION(:,:),ALLOCATABLE :: kx,ky, ux,uy,energy0,EnergyToT
    REAL(num) :: e, T
    ...
    DO iy=0,ny+1
       DO ix=0,nx+1
          bxc=(bx(ix,iy)*dxb(ix)+bx(ix-1,iy)*dxb(ix-1))/(dxb(ix)+dxb(ix-1))
          byc=(by(ix,iy)*dyb(iy)+by(ix,iy-1)*dyb(iy-1))/(dyb(iy)+dyb(iy-1))
          B=SQRT(bxc**2+byc**2+bz(ix,iy)**2)
          !Direction of magnetic field
          ux(ix,iy)=bxc**2/(B**2+b_min**2)
          uy(ix,iy)=byc**2/(B**2+b_min**2)
          !Kappa along magnetic field
          kx(ix,iy)=kappa * (EnergyToT(ix,iy) * energy0(ix,iy))**pow * (ABS(ux(ix,iy))+b_min**2/(B**2+b_min**2))
          ky(ix,iy)=kappa * (EnergyToT(ix,iy) * energy0(ix,iy))**pow * (ABS(uy(ix,iy))+b_min**2/(B**2+b_min**2))
       ENDDO
    ENDDO
    ...
    DO cycle=0,500
       ...
       DO iy=1,ny
          ...
          DO ix=start_index, nx!, 2
             ...
             Rx=QpX*EnergyToT(ix+1,iy) * energy(ix+1,iy) - QmX*EnergyToT(ix-1,iy) * energy(ix-1,iy)             
             Ry=QpY*EnergyToT(ix,iy+1) * energy(ix,iy+1) - QmY*EnergyToT(ix,iy-1) * energy(ix,iy-1)

             Rxx=MpX*EnergyToT(ix+1,iy) * energy(ix+1,iy) + MmX*EnergyToT(ix-1,iy) * energy(ix-1,iy)
             Ryy=MpY*EnergyToT(ix,iy+1) * energy(ix,iy+1) + MmY*EnergyToT(ix,iy-1) * energy(ix,iy-1)


             Rxy=QpY/16.0_num*(QpX*EnergyToT(ix+1,iy+1) * energy(ix+1,iy+1) - QmX*EnergyToT(ix-1,iy+1) * energy(ix-1,iy+1) &
                  + Q0X*EnergyToT(ix,iy+1) * energy(ix,iy+1)) &
                  - QmY/16.0_num*(QpX*EnergyToT(ix+1,iy-1) * energy(ix+1,iy-1) - QmX*EnergyToT(ix-1,iy-1) * energy(ix-1,iy-1)&
                  + Q0X*EnergyToT(ix,iy-1) * energy(ix,iy-1)) &
                  + Q0Y/16.0_num*(QpX*EnergyToT(ix+1,iy) * energy(ix+1,iy)-QmX*EnergyToT(ix-1,iy) * energy(ix-1,iy))
             ...
             A1=M0X*ux(ix,iy)*kx(ix,iy) + M0Y*uy(ix,iy)*ky(ix,iy) +&
                  Q0X*Q0Y/16.0_num*(ux(ix,iy)*ky(ix,iy)+uy(ix,iy)*kx(ix,iy))
             ...
             A1 = A1 + (kx(ix,iy)*M0X + ky(ix,iy)*M0Y + KxX*Q0X + KyY*Q0Y) * b_min**2/(b**2+b_min**2)
             ...
             Q=energy(ix,iy)
             energy(ix,iy)=(1.0_num-w)*energy(ix,iy) + w/(1.0_num/EnergyToT(ix,iy)+A1)*(energy0(ix,iy)/EnergyToT(ix,iy)+A2/EnergyToT(ix,iy))
             Q=(Q-energy(ix,iy))/Q
    ...
    IF (.NOT. converged) PRINT *,"Solution failed to converge during heat conduction", errtot
