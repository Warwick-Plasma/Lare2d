...
!!$    ENDDO
  ...
  SUBROUTINE open_files
    ...
    IF (rank == 0) THEN
       ...
       IF (ios .NE. 0) THEN
          PRINT *,"Unable to open file lare2d.dat for writing. This is most commonly caused by the output directory not existing"
          PRINT *," "
          PRINT *," "
  ...
  SUBROUTINE restart_data
    ...
    DO ix=1,nBlocks
	  ...
	  IF (nd /= DIMENSION_2D) THEN
	     IF (rank == 0) PRINT *,"Non 2D Dataset found in input file, ignoring and continuting."
             CALL cfd_Skip_Block()
             CYCLE
	  ENDIF
