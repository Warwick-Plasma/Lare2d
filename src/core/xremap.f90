...
CONTAINS
  ...
  SUBROUTINE remap_x !remap onto original Eulerian grid
    ...
    DO iy = -1, ny+2
       ...
       DO ix = -1, nx+2
          ...
          cv1(ix,iy) = cv(ix,iy) * (1.0_num + dv)  !control volume before remap
          dv = REAL(ypass,num) * (vyb - vybm)/dyb(iy) * dt
          cv2(ix,iy) = cv(ix,iy) * (1.0_num + dv)  !control volume after remap
          dxb1(ix,iy) = dxb(ix) + (vxb - vxbm) * dt !dxb before remap
       END DO
    END DO
    DO iy = -1, ny+1
          !DEC$ IVDEP
    ...
    DO iy = 1, ny
       ...
       DO ix = 1, nx
          ixm = ix - 1
          rho(ix,iy) = (rho1(ix,iy)*cv1(ix,iy) + dm(ixm,iy)-dm(ix,iy))  &
               /cv2(ix,iy)
       END DO
    END DO
    ...
    ! redfine dxb1,cv1,cv2,dm and vx1 for velocity (vertex) cells
    ...
    DO iy = 0, ny
       ...
       END DO
    END DO
    cv1(0:nx,0:ny) = flux(0:nx,0:ny) / 4.0_num  ! cv1 = vertex CV before remap
    ...
    DO iy = 0, ny
       ...
       END DO
    END DO
    cv2(0:nx,0:ny) = flux(0:nx,0:ny) / 4.0_num  ! cv2 = vertex CV after remap
    ...
    DO iy = 0, ny
       ...
       END DO
    END DO
    vx1(-2:nx+1,0:ny) = flux(-2:nx+1,0:ny)   ! vertex velocity used in remap
    ...
    DO iy = 0, ny
       ...
       END DO
    END DO
    dm(-1:nx,0:ny) = flux(-1:nx,0:ny) / 4.0_num  ! mass flux out of vertex CV
    ...
    DO iy = 0, ny
       ...
       DO ix = 0, nx        ! vertex density after remap
          ixm = ix - 1
          rho_v1(ix,iy) = (rho_v(ix,iy)*cv1(ix,iy)+dm(ixm,iy)-dm(ix,iy)) &
               /cv2(ix,iy)
       END DO
    END DO
  ...
  SUBROUTINE vx_by_flux
    ...
    DO iy = 0, ny
       ...
       DO ix = 0, nx
          ...
          db = (dxb1(ix,iy)+dxb1(ix,iyp)) / 2.0_num
          dbxp = (dxb1(ixp,iy)+dxb1(ixp,iyp)) / 2.0_num
          dbxp2 = (dxb1(ixp2,iy)+dxb1(ixp2,iyp)) / 2.0_num
          dbxm = (dxb1(ixm,iy)+dxb1(ixm,iyp)) / 2.0_num
          w4 = by(ix,iy) / db
          w5 = by(ixp,iy) / dbxp
          ...
          w6 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dxb1(ix,iy)*vad_p&
               +dxb1(ixp,iy)*vad_m), ABS(w1),ABS(w2*vad_p+w3*vad_m))
          flux(ix,iy) = flux(ix,iy) + bzone(ix,iy) * v_advect * dt * w6 * (1.0_num - w5)
  ...
  SUBROUTINE vx_bz_flux
    ...
    DO iy = 1, ny
       ...
       DO ix = 0, nx
          ...
          v_advect = (vx1(ix,iy) + vx1(ix,iym)) / 2.0_num
  ...
  SUBROUTINE x_mass_flux
    ...
    DO iy = 0, ny+1
       ...
       DO ix = 0, nx
          ...
          w4 = w4 / 6.0_num
          w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p+w3*vad_m))
          w6 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dxb1(ix,iy)*vad_p+dxb1(ixp,iy)*vad_m)&
               , ABS(w1), ABS(w2*vad_p+w3*vad_m))
          flux_rho = v_advect * dt * w6 * (1.0_num - w5)

          dm(ix,iy) = (bzone(ix,iy) * flux_rho + dm(ix,iy)) * dyb(iy)
       END DO
    END DO
  ...
  SUBROUTINE x_energy_flux ! energy remap in mass coordinates
    ...
    DO iy = 0, ny
       ...
       DO ix = 0, nx
          ...
          w4 = w4 / 6.0_num
          w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p+w3*vad_m))
          w5 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dxb(ix)*vad_p+dxb(ixp)*vad_m)&
               , ABS(w1), ABS(w2*vad_p+w3*vad_m))
    ...
    IF (rke) THEN
       DO iy = 0, ny
          ...
          DO ix = 0, nx-1
             ...
             dk = (vy(ixp,iy) - vy(ix,iy))  &
                  * (flux(ix,iy) - 0.5_num*(vy(ixp,iy)+vy(ix,iy)))  &
    ...
    IF (rke) THEN
       DO iy = 0, ny
          ...
          DO ix = 0, nx-1
             ...
             dk = (vz(ixp,iy) - vz(ix,iy))  &
                  * (flux(ix,iy) - 0.5_num*(vz(ixp,iy)+vz(ix,iy)))  &
    ...
    IF (rke) THEN
       DO iy = 0, ny
          ...
          DO ix = 0, nx-1
             ...
             dk = (vx(ixp,iy) - vx(ix,iy))   &
                  * (flux(ix,iy) - 0.5_num*(vx(ixp,iy)+vx(ix,iy)))  &
  ...
  SUBROUTINE dm_x_bcs
    ...
    CALL MPI_SENDRECV(dm(1,0:ny+1), ny+2, mpireal, left, tag,  &
         dm(nx+1,0:ny+1), ny+2, mpireal, right, tag, comm, status, errcode)
    IF (right == MPI_PROC_NULL) dm(nx+1,0:ny+1) =  dm(nx,0:ny+1)

    CALL MPI_SENDRECV(dm(nx-1,0:ny+1), ny+2, mpireal, right, tag,  &
         dm(-1,0:ny+1), ny+2, mpireal, left, tag, comm, status, errcode)
    IF (left == MPI_PROC_NULL) dm(-1,0:ny+1) =  dm(0,0:ny+1)
