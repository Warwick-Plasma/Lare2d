...
CONTAINS
  ...
  SUBROUTINE remap_y
    ...
    DO iy = 1, ny
       DO ix = 1, nx
          iym = iy - 1
          rho(ix,iy) = (rho1(ix,iy)*cv1(ix,iy) + dm(ix,iym)-dm(ix,iy))  &
               /cv2(ix,iy)
       END DO
    END DO
    ...
    DO iy = 0, ny
       DO ix = 0, nx
          iym = iy - 1
          rho_v1(ix,iy) = (rho_v(ix,iy)*cv1(ix,iy) + dm(ix,iym)-dm(ix,iy)) &
               /cv2(ix,iy)
       END DO
    END DO
    ...
    IF (rke) THEN
       DO iy = 0, ny-1
          DO ix = 0, nx
             ...
             dk =   (vx(ix,iyp) - vx(ix,iy))   &
                  * (flux(ix,iy) - 0.5_num*(vx(ix,iyp)+vx(ix,iy)))  &
    ...
    IF (rke) THEN
       DO iy = 0, ny-1
          DO ix = 0, nx
             ...
             dk =   (vz(ix,iyp) - vz(ix,iy))   &
                  * (flux(ix,iy) - 0.5_num*(vz(ix,iyp)+vz(ix,iy)))  &
    ...
    IF (rke) THEN
       DO iy = 0, ny-1
          DO ix = 0, nx
             ...
             dk = (vy(ix,iyp) - vy(ix,iy))   &
                  * (flux(ix,iy) - 0.5_num*(vy(ix,iyp)+vy(ix,iy)))  &
  ...
  SUBROUTINE dm_y_bcs
    ...
    CALL MPI_SENDRECV(dm(0:nx+1,1), nx+2, mpireal, down, tag,  &
         dm(0:nx+1,ny+1), nx+2, mpireal, up, tag, comm, status, errcode)
    IF (up == MPI_PROC_NULL) dm(0:nx+1,ny+1) = dm(0:nx+1,ny)
    CALL MPI_SENDRECV(dm(0:nx+1,ny-1), nx+2, mpireal, up, tag,  &
         dm(0:nx+1,-1), nx+2, mpireal, down, tag, comm, status, errcode)
    IF (down == MPI_PROC_NULL) dm(0:nx+1,-1) = dm(0:nx+1,0)
