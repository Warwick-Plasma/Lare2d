...
CONTAINS
  ...
  ! used by LARE. These are expressed in normalised units. 

  SUBROUTINE get_pressure(rho_in, en_in, m_in, ix, iy, p)
    ...
    IF (m_in .EQ. EOS_ION) THEN
      p = (en_in - (1.0_num - xi_n(ix, iy)) * ionise_pot) &
  ...
  SUBROUTINE get_temp(rho_in, energy_in, m_in, ix, iy, temp_out)
    ...
    IF (m_in .EQ. EOS_PI) THEN
      temp_out = (gamma - 1.0_num) * energy_in / (2.0_num - xi_n(ix, iy))
      RETURN
    END IF

    IF (m_in .EQ. EOS_ION) THEN
      temp_out = (gamma - 1.0_num) &
          * (energy_in - (1.0_num - xi_n(ix, iy)) * ionise_pot) &
          / ((2.0_num - xi_n(ix, iy)))
      RETURN
    END IF
  ...
  SUBROUTINE get_cs(rho_in, energy_in, m_in, ix, iy, cs_out)
