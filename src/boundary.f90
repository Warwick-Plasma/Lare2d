...
CONTAINS
  ...
  SUBROUTINE bfield_bcs
    ...
    IF (right == MPI_PROC_NULL .AND. xbc_max == BC_OTHER) THEN
      bx(nx+1, :) = bx(nx-1, :)
      bx(nx+2, :) = bx(nx-2, :)
      by(nx+1, :) = by(nx  , :)
      by(nx+2, :) = by(nx-1, :)
      ...
      bz(nx+2, :) = bz(nx-1, :)
    END IF
    IF (left == MPI_PROC_NULL .AND. xbc_min == BC_OTHER) THEN
      bx(-1, :) = bx(1, :)
      bx(-2, :) = bx(2, :)
      by( 0, :) = by(1, :)
      by(-1, :) = by(2, :)
    ...
    IF (up == MPI_PROC_NULL .AND. ybc_max == BC_OTHER) THEN
      bx(:, ny+1) = bx(:, ny  )
      bx(:, ny+2) = bx(:, ny-1)
      by(:, ny+1) = by(:, ny-1)
      by(:, ny+2) = by(:, ny-2)
      ...
      bz(:, ny+2) = bz(:, ny-1)
    END IF
    IF (down == MPI_PROC_NULL .AND. ybc_min == BC_OTHER) THEN
      bx(:,  0) = bx(:, 1)
      bx(:, -1) = bx(:, 2)
      by(:, -1) = by(:, 1)
      by(:, -2) = by(:, 2)
