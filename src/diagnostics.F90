  ...
  SUBROUTINE output_routines(i)   ! i=step index
    ...
    IF (print_arrays) THEN   ! output a snapshot of arrays
       ...
       IF (DumpMask(1))  THEN
          ...
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("Rho",&
               "Fluid",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(2))  THEN
          Data=energy(0:nx,0:ny)*ENERGY0
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("Energy",&
               "Fluid",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(3))  THEN
          Data=vx(0:nx,0:ny)*VEL0
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("Vx",&
               "Velocity",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(4))  THEN
          Data=vy(0:nx,0:ny)*VEL0
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("Vy",&
               "Velocity",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(5))  THEN
          Data=vz(0:nx,0:ny)*VEL0
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("Vz",&
               "Velocity",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(6))  THEN
       ...
       IF (DumpMask(9)) THEN
          DO iy=0,ny
             DO ix=0,nx
                CALL Get_Temp(rho(ix,iy),energy(ix,iy),eos_number,ix,iy,Data(ix,iy))
             ENDDO
          ENDDO
          Data=Data * TEMP0
       ...
       IF (DumpMask(10))  THEN
          DO iy=0,ny
             DO ix=0,nx
                CALL Get_Pressure(rho(ix,iy),energy(ix,iy),eos_number,ix,iy,Data(ix,iy))
             ENDDO
          ENDDO
          Data=Data * PRESSURE0
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("Pressure",&
               "Fluid",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       ...
       IF (DumpMask(11)) THEN
          DO iy=0,ny
             DO ix=0,nx
                CALL Get_Cs(rho(ix,iy),energy(ix,iy),eos_number,ix,iy,Data(ix,iy))
             ENDDO
          ENDDO
          Data=Data * VEL0
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("cs",&
               "Fluid",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       ...
       IF (DumpMask(12)) THEN
          ...
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("j_par",&
       ...
       IF (DumpMask(13)) THEN
          ...
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("j_perp",&
               "PIP",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       ...
       IF (DumpMask(15)) THEN
          ...
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("eta_perp",&
               "PIP",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       ...
       IF (DumpMask(16)) THEN
          ...
          CALL cfd_Write_2D_Cartesian_Variable_Parallel("eta",&
               "PIP",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
    ...
    DO iy = 0, ny
       DO ix = 0, nx
          ...
          IF (cowling_resistivity) THEN
             dt3 = 0.2_num * dxlocal / MAX(MAX(eta(ix,iy),eta_perp(ix,iy)), none_zero)
          ELSE
